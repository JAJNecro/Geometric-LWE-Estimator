...
nb_tests = 200 # amount of tests to run
ring = 0 # whether to use ringlwe or not (0 = LWE, 1 = RLWE)
...
def one_experiment(seed):
    ...
    Sigma = Sigma * d # need to scale by dimension for toolkit
    ...
    ebdd_norm = (norm)
    our_ebdd.estimate_attack() # predicted hardness estimates
    ebdd_predicted_beta_normal = (our_ebdd.beta)
    our_ebdd.estimate_attack(probabilistic = True) # predicted probabilisitc hardness estimates
    ...
    # Kannan's Embedding (Standard Attack)
    D_s = build_Gaussian_law(sigma, 50)
    D_e = D_s
